...
void ImageCopyCaptureSession::handleDone()
{
  ...
  if (!pickShmFormatCb || !pickShmFormatCb(formats, &shmFormat)) {
    ...
    // FIXME: proper error handling -- don't just return
    return;
  }
  ...
  size = width * height * 4;
  initPool("w0vncserver-image-copy-shm", size);
...
void ImageCopyCaptureSession::initPool(const char* name, size_t size)
{
  int fd;
