...
namespace wayland {
  ...
  public:
    ImageCaptureSource(ext_image_capture_source_v1* source);
    ~ImageCaptureSource();

    ext_image_capture_source_v1* getSource() const { return source; }

  private:
    ext_image_capture_source_v1* source;
  };

  class OutputImageCaptureSourceManager : public Object {
  public:
    OutputImageCaptureSourceManager(Display* display);
    ~OutputImageCaptureSourceManager();

    ImageCaptureSource* createSource(Output* output);
  ...
  class ForeignToplevelImageCaptureSourceManager : public Object {
  public:
    ForeignToplevelImageCaptureSourceManager(Display* display);
    ~ForeignToplevelImageCaptureSourceManager();

    ImageCaptureSource* createSource(ext_foreign_toplevel_handle_v1* toplevel);
